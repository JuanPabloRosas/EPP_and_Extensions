...
def grasp():
   ...
   sol_ls = local_search(sol)				                                    #	BUSQUEDA LOCAL CON LA SOLUCION GENERADA
                                                                           #  SI LA SOLUCION ES MEJOR ACTUALIZA
...
def vns():
   for i in range(0,iteraciones):
      ...
      print('IT:   ' + str(i+1)+'   MEJOR SOL:   ' + str(valor_obj(mejor_sol)))
      k = 1
      while(k <= vecindarios):
         sol_sh = shaking_vns(solucion_inicial.copy(),k)
         nueva_sol = local_search_vns(sol_sh.copy(),k,i)
         if(valor_obj(nueva_sol) <= valor_obj(solucion_inicial)):
            solucion_inicial = nueva_sol.copy()
         else:
            k = k + 1
      if(valor_obj(solucion_inicial) <= valor_obj(mejor_sol)):
         mejor_sol = solucion_inicial.copy()
   return(mejor_sol)


f = open(folder + 'soluciones/SolucionesVNS_GRASP_HIBRIDO/VNS_GRASP_HIBRIDO.csv' , "w")
g = open(folder + 'soluciones/SolucionesVNS_GRASP_HIBRIDO/soluciones_VNS_GRASRIDO.csv' , "w")
for CI in [0.99,0.95,1.03]:
   for cand in range(30,31,10):
      #for iteraciones in range(300,301):
      for iteraciones in range(100,101):
         for filename in os.listdir(folder):
            if filename.endswith(".csv"):
               ...
               for k in range(kmin,101,5):
                  ...
                  f.write(nombre[0] + ',' + nombre[1] + ',' + nombre[2].replace('.csv','') + ',' + str(k) + ',' + str(CI) + ',' + ev + ',' + 'VNS_EPP-SL,' + str(cand) + ',' + str(iteraciones)  + ',' + str(valor_obj(sol_final)) + "," + str(runtime) + "," + str(len(sol_final)) + '\n' )
                  print("CAND: "+ str(cand) + "  ITERACION: " + str(iteraciones) +"  CI: " + str(CI) + "  KMIN: " + str(k) + "  RUNTIME: " + str(runtime) + "  SOL FINAL: " + str(valor_obj(sol_final)))
